                                   ...
                                   `(,c
                                     (with-tryable-applications
                                         ,(reverse (mapcar #'list
                                                           ...
                                                           (append types types)))
                                       #+nil ,(reverse (mapcar #'list function-names types))
                                       (hole ,output-type))))
...
(defun term-size (term &aux (counter 0))
  (labels ((process (term)
    ...
    (process term)
    counter))
(defun replace-subterm (term i f)
  (labels ((process (term)
             (if (zerop i)
                 (funcall f term)
                 (progn
...
(defun generate-mutation (term hole)
  ...
  (labels ((process (term type)
             (choice
              ;; Construction
               (0.3 (bind ((f (choose-list (library-functions type)))
                           (args (mapcar
                                  (lambda (arg-type)
               ...
               (1.0 (t:ematch term
                      ...
                      ((list* op args)
                       (bind (((:values arg i) (choose-list args))
                         ...
                         (choice
                           ;; Projection
                           (1.0 (if (equal arg-type type) arg (fail)))
                           ;; Subterms
                           (1.0 (cons op (replace-nth args i (process arg arg-type))))))))))))
...
(defun solve-skeleton-one-try (skeleton examples library-functions)
  (let ((*library-table* (make-hash-table :test 'equal)))
    (labels ((mutate (par)
               (one-value
                 (bind (((:values hole i) (choose-list (skeleton-holes skeleton))))
