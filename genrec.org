  ...
  (bind ((exprs (mapcar #'car exprs-and-types))
    ...
    `(let ,vars
       (compiler-let
           ((*tryable-vars* ',(append vars *tryable-vars*))
            (*tryable-types* ',(append types *tryable-types*)))
         ,@ (mapcar (lambda (var expr) `(setq ,var ,expr)) vars exprs)
         ,@body))))
...
(defmacro population-skeleton (&rest types)
  (bind ((main-type (lastcar types))
        ...
        `(with-skeleton (:type ,main-type :lazy-evaluation-p t)
           (labels
               ,(mapcar (curry #'single-dispatch-function-binding function-names types)
...
*** Interpreter Skeleton (Single-dispatch function population with open recursion)
#+begin_src lisp
  ...
  (bind ((seed (or *seed* (random most-positive-fixnum)))
    ...
    (catch 'random-search
      (bind ((*edit-distance-memo* (make-edit-distance-memo))
             ...
             (stuck-iterations 0))
        (vom:info "Search w/seed ~a started, cost ~a." seed cost-1)
        (iter
          (while (< n *restart*))
          (incf n)
...
The following are some experiemental variations of the cost function. They are not currently used.
#+begin_src lisp :load no
(defun skeleton-cost (skeleton examples par)
  "Compute the cost given `examples' for the last function in `skeleton'.
`examples' are of the form `(((input ...) expected-output) ...)'.
This weights every example 0.5x comparing to the preceding example.
This is not currently used."
  (let ((cost-error
          (catch 'cost
            (let ((functions (skeleton-functions skeleton par)))
              (iter (for (input output) in examples)
                (for weight initially 1.0 then (* weight 0.5))
                (for output-1 =
                     (with-resource-limit
                       (apply (lastcar functions) input)))
                (clrhash *edit-distance-memo*)
          ...
          (catch 'cost
            (let ((functions (skeleton-functions skeleton par)))
              (iter (for (label input output) in examples)
                (for output-1 =
                     (with-resource-limit
                       (apply (nth label functions) input)))
                (clrhash *edit-distance-memo*)
                (sum (fastlog2
                      (the fixnum
                           (1+ (edit-distance
  ...
  (unwind-protect
       (let ((channel (lparallel:make-channel))
         ...
         (iter
           ...
           (when par (return (values par thunk)))
           (lparallel:submit-task
            channel
            (bind (((:values context-vars context-vals)
                    (capture-context-vars)))
  ...
  (bind ((result)
    ...
    (labels ((do-search ()
               (unless result
                 ...
                 (bind (((:values context-vars context-vals) (capture-context-vars))
                        (form `(progv ',context-vars ,(uneval context-vals)
                                 (solve-skeleton-one-try
                                  ',skeleton ',examples ',library-functions))))
                   (incf *seed*)
                   (catcher
                    (alet ((my-result (invoke-aws-lambda form)))
                      (incf *search-completed*)
                      (when my-result
                        (vom:info "Voila!")
                        (setq result my-result))
                      (do-search))
                    (error (c)
                           (incf *search-failed*)
                           (vom:error "Search failed: ~a" c)
                           (do-search)))))))
