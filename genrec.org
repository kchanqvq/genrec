...
*** Context Vars
**** Motivation
The search algorithm depends on a number of parameters and state variables. Typically, these are treated in either of the following ways, none of which I feel completely satisfactory:
- Pass as =&key= arguments. To propagate required parameter into inner procedures, this requires lots of boilerplates to manually pass =&rest args= argument list.
- Define a =search-state= object/struct, and either
  - store the current =search-state= in a special variable =*search-state*=, or
  - pass around a single argument =search-state= (relevant procedures now have type =(function (search-state) search-state)=).
  Disadvantage: have to manually write lots of accessors (=(search-state-some-slot *search-state*)=). Possible workaround: use a macro to generate =define-symbol-macro='s for each slots, but this is just simulating the =context-vars= facility in a more complicated way.
- Store the parameters/states in some special variables. This is the basis of the =context-vars= facility, and using interesting states/variables is very convenient (just normal variable access, no need to manually pass anything).

The =context-vars= facility solves the following problems not provided by vanilla special variables:
- Capture/save/restore the "dynamic context" (the set of current parameter/states). Useful for:
  - Transport parameter/state to remote Lisp images
  - Use the search procedures in a functional/reproducible way (wrap with =with-save-context-vars=, or bind input parameter/states with =progv=)
- Convert between special variable bindings and plists (=&key= argument lists)
**** Implementation
#+begin_src lisp
(defvar *context-vars*
  '(*beta* *search-procedure* *n-accepted* *max-accepted* *max-stuck-iterations*
    *seed* *prng* *frozen-holes* *par* *cost* *solved-p* *eval-toplevel*))
(defun capture-context-vars ()
  (iter
    (for s in *context-vars*)
    (unless (equalp (symbol-value s)
                    (sb-ext:symbol-global-value s))
      (collect s into vars)
      (collect (symbol-value s) into values))
    (finally (return (values vars values)))))
(defun context-var-to-keyword (symbol)
  (assert (eq (symbol-package symbol) (find-package "GENREC")))
  (bind ((name (symbol-name symbol)))
    (assert (eq (aref name 0) #\*))
    (assert (eq (aref name (1- (length name))) #\*))
    (intern (subseq name 1 (1- (length name))) "KEYWORD")))
(defun keyword-to-context-var (keyword)
  (assert (eq (symbol-package keyword) (find-package "KEYWORD")))
  (or (find-symbol (s:concat "*" (symbol-name keyword) "*") "GENREC")
      (error "Unknown keyword ~a for context var." keyword)))
(defun context-vars-plist ()
  (bind (((:values context-vars context-vals)
          (capture-context-vars)))
    (iter (for s in context-vars)
      (for v in context-vals)
      (nconcing (list (context-var-to-keyword s) v)))))
(defun plist-context-vars (plist)
  (iter (for (k v) on plist by #'cddr)
    (set (keyword-to-context-var k) v)))
(defmacro with-saved-context-vars ((&optional context-bindings) &body body)
  `(progv *context-vars* (mapcar #'symbol-value *context-vars*)
     ,(when context-bindings `(plist-context-vars ,context-bindings))
     ,@body))
#+end_src
** Type utilities
...
(defmacro choice (&body clauses)
  (let ((norm (apply #'+ (mapcar #'car clauses)))
        (cdf 0.0))
    `(let ((%random-choice (random-state:random-float *prng* 0.0 ,norm)))
...
(defun choose-list (list)
  (if list
      (let ((i (random-state:random-int *prng* 0 (1- (length list)))))
        (values (nth i list) i))
      (fail)))
#+end_src
*** Generate mutations
...
(locally
  ...
  (defun accept-p (cost-2 cost-1)
    ...
    (or (< cost-2 cost-1)
        (< (+ (fastlog2 (1+ (the fixnum (random-state:random-int *prng* 0 1000))))
              (* *beta* (- cost-2 cost-1)))
           (fastlog2 (1+ 1000))))))
#+end_src
...
(defvar *n-accepted* 0 "Current number of accepted proposals.")
(defvar *max-accepted* 500000 "Stop the search when `*n-accepted*'=`*max-accepted*'.")
(defvar *par* nil "Current candidate parameter.")
(defvar *cost* nil "Current candidate cost.")
(defvar *solved-p* nil)
#+end_src

The following perform one single trace of MCMC search, stopping after =*n-accepted*= reaches =*max-accepted*=.. Users are expected to have some driver loop that calls it repeatedly until success.
...
(defun mcmc-search-one-step (mutate cost)
  "Increments `*n-accepted*' if succesful."
  (iter (for i below *max-stuck-iterations*)
    (bind ((new-par (funcall mutate *par*))
           ((:values new-cost correct-p) (funcall cost new-par)))
      (when (or correct-p (funcall *accept-p* new-cost *cost*))
        (incf *n-accepted*)
        (psetf *cost* new-cost *par* new-par *solved-p* correct-p)
        (return)))
    (finally
        (vom:info "Stuck on ~a for ~a iterations, give up." *par* i))))

(defun mcmc-search (mutate cost)
  (ensure *cost* (funcall cost *par*))
  (vom:info "MCMC seed=~a started, cost ~a." *seed* *cost*)
  (iter
    (while (< *n-accepted* *max-accepted*))
    (for n-before = *n-accepted*)
    (mcmc-search-one-step mutate cost)
    (while (> *n-accepted* n-before)) ;; Otherwise `mcmc-search-one-step' fails
    (until *solved-p*)
    (finally-protected
     #+nil (print *par*)
     (vom:info "MCMC seed=~a stopped, ~a accepted proposals, cost ~a."
               *seed* *n-accepted* *cost*))))
#+end_src

#+begin_src lisp
(defun greedy-search (mutate cost)
  (let ((*accept-p* #'<))
    (mcmc-search mutate cost)))
#+end_src
...
(defun solve-skeleton-one-try (skeleton examples library-functions &rest context-bindings)
  ...
  (with-saved-context-vars (context-bindings)
    (ensure *prng* (random-state:make-generator 'random-state:pcg *seed*))
    (ensure *par* (mapcar (compose #'default-term #'hole-type) (skeleton-holes skeleton)))
    (bind ((*library-table* (make-hash-table :test 'equal))
      ...
      (labels ((mutate (par)
        ...
        (mapc
         ...
         library-functions)
        (funcall *search-procedure* #'mutate #'cost)
        (context-vars-plist)))))
#+end_src
...
Each thunk is called with no argument, and should return a plist from `context-vars-plist'.
The search stop if any thunk returns a plist with `*solved-p*' bind to true, and this plist is returned."
  (unwind-protect
       (let ((channel (lparallel:make-channel))
             (lparallel:*task-category* 'parallel-search)
             (*seed* *seed*)
             (n-concurrency (ceiling (lparallel:kernel-worker-count) (length thunks))))
         (labels ((submit-task (thunk)
           ...
           (iter (for _ below n-concurrency)
             ...
             (incf *seed*))
           (iter
             (for (thunk . result-bindings) = (lparallel:receive-result channel))
             (when (getf result-bindings :solved-p)
...
(defun solve-skeleton (skeleton examples library-functions &rest context-bindings)
  (with-saved-context-vars (context-bindings)
  ...
  (with-standard-io-syntax
    (alet ((response
            (carrier:request
             ...
             :body (write-to-string form :readably t :escape t :circle t)
             :return-body t
             :ssl-ctx *ssl-ctx*)))
...
#-aws-lambda
(defvar *time-slice-n-accepted* 100000)
#-aws-lambda
(defun aws-lambda-solve-skeleton (skeleton examples library-functions)
  ...
  (bind (result)
    ...
    (labels ((do-search (context-bindings)
               (unless result
                 (setf (getf context-bindings :max-accepted)
                       (+ (getf context-bindings :n-accepted 0)
                          *time-slice-n-accepted*))
                 (incf *search-launched*)
                 (bind ((form `(apply #'solve-skeleton-one-try
                                      ',skeleton ',examples ',library-functions
                                      ,(uneval context-bindings))))
                   (catcher
                    (alet ((result-bindings (invoke-aws-lambda form)))
                      #+nil (print (list seed next-seed initial-par par))
                      (incf *search-completed*)
                      (when (getf result-bindings :solved-p)
                        (vom:info "Search ~a succeeded"
                                  (getf context-bindings :seed))
                        (setq result result-bindings))
                      (when (< (getf context-bindings :max-accepted)
                             *max-accepted*)
                          (do-search result-bindings)))
                    (error (c)
                           (incf *search-failed*)
                           (vom:error "Search ~a failed: ~a"
                                      (getf context-bindings :seed) c)))))))
...
#+begin_src lisp
(defun sample (type)
  (let ((op (nth (random-state:random-int *prng* 0 (1- (length (constructors type))))
                 (constructors type))))
    (if (fboundp op)
